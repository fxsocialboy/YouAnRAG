"""Generate phase-2 closing report and artifact README."""
...
def generate_phase2_report(project_root: Path = PROJECT_ROOT) -> tuple[Path, Path]:
    cfg = get_config()
    stage2_dir = cfg.stage2_artifacts_dir
    docs_dir = project_root / "docs"
    docs_dir.mkdir(parents=True, exist_ok=True)

    import_report = read_json(stage2_dir / "qdrant_import_report.json")
    scan_report = read_json(stage2_dir / "sync_scan_report.json")
    add_report = read_json(stage2_dir / "sync_test_add_report.json")
    modify_report = read_json(stage2_dir / "sync_test_modify_report.json")
    delete_report = read_json(stage2_dir / "sync_test_delete_report.json")
    clean_scan_report = read_json(stage2_dir / "sync_test_clean_scan_report.json")
    consistency_eval = read_json(project_root / "experiments" / "stage2_consistency_results.json")
    consistency_summary = consistency_eval.get("summary", {})

    registry = DocumentRegistry(cfg.registry_db_path)
    registry_docs = registry.list_documents(include_deleted=True) if cfg.registry_db_path.exists() else []
    active_docs = [item for item in registry_docs if item.status != "deleted"]
    deleted_docs = [item for item in registry_docs if item.status == "deleted"]

    phase2_report = docs_dir / "phase2_report.md"
    artifact_readme = stage2_dir / "README.md"

    phase2_text = f"""# 阶段二收口报告：Qdrant Local 接入与 Markdown 自动更新
...
- Qdrant point 数：{import_report.get('qdrant_count', 'N/A')}；
- Stage1 metadata 数：{import_report.get('metadata_count', 'N/A')}；
- point 数与 metadata 数一致：{import_report.get('qdrant_count') == import_report.get('metadata_count') if import_report else 'N/A'}；
...
```json
{json.dumps(scan_report.get('changes', {}), ensure_ascii=False, indent=2)}
```
...
```json
{json.dumps(add_report, ensure_ascii=False, indent=2)}
```

修改：

```json
{json.dumps(modify_report, ensure_ascii=False, indent=2)}
```

删除：

```json
{json.dumps(delete_report, ensure_ascii=False, indent=2)}
```

清理后再次 scan：

```json
{json.dumps(clean_scan_report, ensure_ascii=False, indent=2)}
```
...
- 修改后旧内容被替换，新内容可检索；
- 删除后 Qdrant point 数恢复到 {import_report.get('qdrant_count', 'N/A')}；
- 清理后 `test_sync.md` 不再被 scan 发现；
- 当前 Registry 状态：active 文档 {len(active_docs)} 条，deleted 文档 {len(deleted_docs)} 条。

## 6. Stage1 vs Stage2 一致性评测摘要

```json
{json.dumps(consistency_summary, ensure_ascii=False, indent=2)}
```

关键结论：

- 25 条 query 的 Top-10 顺序一致率均值：{consistency_summary.get('avg_top10_prefix_match_ratio', 'N/A')}；
- 25 条 query 的 Top-10 集合重合率均值：{consistency_summary.get('avg_top10_set_overlap_ratio', 'N/A')}；
- payload 完整率：{consistency_summary.get('payload_completeness_ratio', 'N/A')}；
- filter 正确率：{consistency_summary.get('filter_pass_ratio', 'N/A')}；
- Qdrant point 数与 metadata 数完全对齐：{consistency_summary.get('point_count_matches_metadata', 'N/A')}。
...
"""
    ...
    artifact_text = f"""# Stage2 Artifacts
...
```json
{json.dumps({
    'qdrant_point_count': consistency_summary.get('qdrant_point_count'),
    'metadata_count': consistency_summary.get('metadata_count'),
    'point_count_matches_metadata': consistency_summary.get('point_count_matches_metadata'),
    'avg_top10_prefix_match_ratio': consistency_summary.get('avg_top10_prefix_match_ratio'),
    'payload_completeness_ratio': consistency_summary.get('payload_completeness_ratio'),
    'filter_pass_ratio': consistency_summary.get('filter_pass_ratio'),
    'active_registry_document_count': len(active_docs),
    'deleted_registry_document_count': len(deleted_docs),
...
"""
    ...
    phase2_report.write_text(phase2_text, encoding="utf-8")
    artifact_readme.write_text(artifact_text, encoding="utf-8")
    return phase2_report, artifact_readme


def main() -> None:
    phase2_report, artifact_readme = generate_phase2_report()
    print(f"wrote {phase2_report}")
